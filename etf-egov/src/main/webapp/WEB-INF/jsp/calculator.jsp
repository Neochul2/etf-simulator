...
<body>
	...
	<nav class="navbar navbar-expand bg-white border-bottom mb-4">
		<div class="container">
			<a class="navbar-brand"
				href="<%=request.getContextPath()%>/etf/list.do">📊 월배당 ETF</a>
	...
	<div class="container">
			...
			<div class="card-body d-flex align-items-center">
				...
				<div>
					...
					<span class="badge bg-primary" id="symbolBadge"></span> <span
						class="badge bg-light text-dark" id="tagArea"
						style="display: none;">미국 NASDAQ USD</span>
				</div>
			</div>
		</div>
			...
			<div class="col-md-6">
				...
				<div class="card shadow-sm mb-3">
					<div class="card-body">
						...
						<label class="form-label">투자금액 (원)</label> <input type="number"
							id="investAmount" class="form-control" value="10000000">
						<small class="text-muted" id="usdConverted">≈ $0.00 USD</small>

						<%-- 계산하기 버튼 — 투자금액 입력창 바로 아래로 이동 --%>
						<button class="btn btn-primary w-100 py-2 mt-3" id="calculateBtn">🧮
							계산하기</button>
					</div>
				</div>

				<div class="alert alert-light border small">
					ℹ️ 안내<br> · 환율은 1일 1회 업데이트됩니다.<br> · 투자금액은 원화 기준으로
					입력해주세요.
				</div>
			</div>
		...
		<div class="alert alert-light border small my-4">
			ℹ️ 안내<br> · 배당금은 최근 12개월 지급 내역 기준으로 산출되며, 배당률 및 환율 변동에 따라 달라질 수
			있습니다.<br> · 실제 투자 결과와 다를 수 있습니다.
		</div>
...
// 종목 정보만 불러올 뿐, 자동으로 계산은 실행하지 않음 (버튼 눌러야만 계산)
...
// 입력값 실시간 USD 환산만 표시 (계산은 안 함)
document.getElementById('investAmount').addEventListener('input', function() {
    // 필요 시 USD 환산만 미리 보여주고 싶다면 여기서 처리 가능 (현재는 calculate.do 응답에서 표시)
});
...
function renderResult(r) {
    ...
    document.getElementById('afterTaxYield').innerText = (currentDivYield * (1 - 0.154)).toFixed(2) + '%';
