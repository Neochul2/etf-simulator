...
| 프레임워크 | eGovFrame 4.3.1 / FastAPI 0.115 |
| ORM | MyBatis 3.5.15 |
| 데이터베이스 | MySQL 8.0 |
| 배포 서버 | Apache Tomcat 9.0 / uvicorn + systemd |
| 인프라 | Oracle Cloud VM (Ubuntu 24.04) |
| 외부 API | Polygon.io (주가·배당) / 한국수출입은행 (USD/KRW 환율) |
| 자동화 | crontab + Telegram Bot 알림 |
...
| 테이블 | 설명 |
|---|---|
| `etf_info` | ETF 기본정보 (현재가·OHLCV·배당률·설명) |
...
└── etf-egov/                             # Java 화면 서버 (eGovFrame)
    └── src/main/
        ...
        │   ├── service/impl/
        │   │   ├── DividendCalculatorServiceImpl.java
        │   │   ├── DripCalculatorServiceImpl.java
...
|---|---|---|
| GET | `/` | 서버 상태 확인 |
| GET | `/etf/list` | ETF 전체 목록 조회 (배당률 내림차순) |
| GET | `/etf/{ticker}` | ETF 상세 조회 (가격·OHLCV·배당률·설명) |
| GET | `/etf/{ticker}/dividends` | ETF 배당 내역 조회 (최근 12개월) |
...
eGovFrame 컨트롤러는 RestTemplate으로 FastAPI를 호출하며, FastAPI 장애 시 MyBatis로 DB를 직접 조회하는 fallback 로직이 적용되어 있습니다.

```
EtfController       -> GET /etf/{ticker}           -> EtfInfoVO
                    -> GET /etf/{ticker}/dividends  -> EtfDividendVO[]
                    -> [실패 시] MyBatis DB 직접 조회 (fallback)

CalculatorController -> GET /etf/{ticker}           -> divYield 취득
                     -> GET /exchange/usdkrw        -> 환율 취득
                     -> DividendCalculatorService.calculate() 호출

ExchangeController   -> POST /exchange/update       -> 수출입은행 API 트리거
```

FastAPI polygon.py 응답 키는 Java VO 필드명(camelCase)과 일치하도록 변환하여 RestTemplate 역직렬화가 자동으로 동작합니다.
